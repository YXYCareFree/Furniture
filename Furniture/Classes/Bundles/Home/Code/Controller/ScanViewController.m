...
- (void)interfaceInit{
    ...
    UITapGestureRecognizer * tap = [[UITapGestureRecognizer alloc] initWithTarget:self.interactor action:@selector(backBtnClicked)];
    [bgView addGestureRecognizer:tap];
    [self.view addSubview:bgView];
    
    if (IS_IPAD || IS_IPAD_PRO) {
        _lightBtn = [[UIButton alloc] initWithFrame:CGRectMake(kScreenWidth / 2 - 60, kScreenHeight / 2 - 200 - 50, 120, 40)];
    }else{
    ...
    if (IS_IPAD || IS_IPAD_PRO) {
        _scanView = [[UIImageView alloc] initWithFrame:CGRectMake((kScreenWidth - 300) / 2, kScreenHeight / 2 - 150 - 50, 300, 300)];
    }else{
    ...
    if (IS_IPAD || IS_IPAD_PRO) {
        _line = [[UIImageView alloc] initWithFrame:CGRectMake(kScreenWidth/ 2 - 130, kScreenHeight / 2 - 350, 260, 2)];
    }else{
...
//150--450
    if (IS_IPAD || IS_IPAD_PRO) {
        ...
        if (_upOrDown == NO) {
            _num ++;
            _line.frame = CGRectMake(kScreenWidth/ 2 - 130, kScreenHeight / 2 - 180 + 2 * _num, 260, 2);
            if (2 * _num >= 260) {
                _upOrDown = YES;
            }
        }else {
