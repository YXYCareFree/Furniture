...
- (id)init
{
    if ((self = [super init])){
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(setStatusBarHidden:) name:StatusBarHidden object:nil];
    }
    
    return self;
}

- (void)setStatusBarHidden:(NSNotification *)notifi{
    _statusBarHidden = [notifi.object boolValue];
}
